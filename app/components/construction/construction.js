...
angular.module('construction', ['ngRoute'])
    ...
    .controller('ConstructionCtrl', ['$route', '$rootScope', '$scope', '$location', 'Helpers', 'Country',
        function ($route, $rootScope, $scope, $location, Helpers, Country) {
            if ($rootScope.token.length < 1) {
                $location.path('login');
            } else {
                var buildings = $rootScope.buildings;
